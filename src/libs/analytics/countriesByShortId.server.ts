...
export async function buildCountriesByShortIdQuery(shortId: string, limit: number = 10, userId: string): Promise<IBuildCountriesQueryResult[]> {
    const query = `
        SELECT
    ...
    const result = await env.DB.prepare(query).bind(shortId, userId, limit).all<{ country: string; clicks: number }>();
